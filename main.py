...
def Action(): #Выполнение действий, основываясь на направлении
    if sum_ == 2 or sum_ == 3 or sum_ == 0: #Комната находится слева?
        PD_line_l_x(2, 1.2, 400, 300, 30, 20, 155, 110, 40, -150) #ПРИМЕРНОЕ ЗНАЧЕНИЕ
        ...
        PD_line_r_x(2, 1.2, 400, 300, 30, 20, 155, 110, 40, -150) #ПРИМЕРНОЕ ЗНАЧЕНИЕ
...
"""
timer_C = time.time()
while (time.time() - timer_C) <= 60:
    right_array.append(hitech_r.read('COLOR')[0])
    ev3.screen.print(hitech_r.read('COLOR')[0])
    wait(100)
    ev3.screen.clear()

print(right_array)
"""
PD_line_l_x(2, 1.2, 400, 300, 30, 20, 155, 110, 40, -150)
