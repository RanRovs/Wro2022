...
def Colr_Thing_l(Time): #Считывание бельевого блока слева
    ev3.speaker.beep()
    timing = time.time()
    while time.time() - timing < Time:
        left_array.append(hitech_l.read('COLOR')[0])
...
def White_room_l(): #Дествия для левой белой комнаты
    ...
    PD_time(2, 1.1, 350, 0.40)#Подъезд к столику для выставления бутылок
    motor_b.run_time(-960, 2000, wait=False)
    Move(300, 0, 200, 0)
    motor_r.stop()
    motor_l.stop()
    ...
    Move(0, 400, 0, 160)
    Move(-200, 0, 60, 0)
    global Err_old#Начинается езда по линии
    while color_l.reflection() <= 50:
        Err = 50 - color_r.reflection()
        motor_l.run(-1 * (300 - (4 * Err + 3 * (Err - Err_old))))
        motor_r.run(300 + (4 * Err + 3 * (Err - Err_old)))
        Err_old = Err
        wait(50)
    ev3.speaker.beep()
    motor_r.stop()
...
def Action(): #Выполнение действий, основываясь на направлении
    if sum_ == 2 or sum_ == 3 or sum_ == 0: #Комната находится слева?
        PD_line_l_x(2, 1.2, 600, 300, 30, 20, 125, 110, 40, -150)
        ...
        PD_line_r_x(2, 1.2, 600, 300, 30, 20, 125, 110, 40, -150)
...
Move(-400, -400, 180, 180)#Отъезд назад после взятия
#Первая бутылка
motor_r.hold()
motor_l.hold()
...
motor_r.stop()
motor_l.stop()
motor_b.run_time(-200, 1000)
Move(300, 0, 360, 0)
#Бутылки взяты

PD_time(2, 1.2, 400, 0.5)
ev3.speaker.beep()
PD_line_r_x(2, 1.2, 600, 350, 35, 20, 125, 110, 40, -50)    #Саша, есть предположение, что я переборщил с количеством параметров...

PD_line_l_x(2, 1.2, 600, 350, 30, 20, 125, 110, 40, -150)
#Запускается параллельное считывание цветов
left_array = []
right_array = []
Colr(2, 1.2, 600, 0.7, 0.4)
print(right_array)
print("Ebat")
print(left_array)
ev3.speaker.beep()
